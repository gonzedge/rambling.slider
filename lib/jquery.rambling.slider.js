  ...
  */
  ...
  (function($) {
    ...
    return RamblingSlider = function(element, options) {
      var addCaption, addControlNavigation, addDirectionNavigation, addKeyboardNavigation, animateBoxes, animateBoxesIn2d, animateFullImage, animateSlices, animationTimeBuffer, children, createBoxes, createSlices, extendAvailableTransitions, fadeBoxes, fadeSlices, flashTransitions, foldSlices, getAnimationHelpers, getAnimationsForCurrentSlideElement, getAvailableTransitions, getBox, getOneSlice, getRamblingBox, getRamblingSlice, getRandomAnimation, getSettingsArrayFor, getSlice, growBoxes, imageFlashTransitions, imageTransitions, pauseSlider, prepareAdaptiveSlider, prepareAnimationContainer, prepareSliderChildren, processCaption, rainBoxes, raiseAnimationFinished, ramblingRun, resetTimer, setAnimationFinishedActions, setCurrentSlideElement, setSliderBackground, setSliderInitialState, setUpTransitions, settings, slideDownSlices, slideTo, slideUpDownSlices, slideUpSlices, slider, timer, transitionGroupSuffixes, transitionGroups, unpauseSlider, vars;
      slider = $(element);
      ...
      createBoxes = function(boxCols, boxRows) {
        var animationContainer, boxHeight, boxWidth, rows, _fn;
        if (boxCols == null) {
      ...
      getSlice = function(sliceWidth, position, total, vars, slideElement) {
        ...
        sliceCss = {
          ...
          width: position === (total - 1) ? slider.width() - (sliceWidth * position) : sliceWidth,
          height: 0,
          opacity: '0',
          overflow: 'hidden'
        };
      ...
      getBox = function(boxWidth, boxHeight, row, column, settings, vars) {
        ...
        boxCss = {
          ...
          width: column === (settings.boxCols - 1) ? slider.width() - (boxWidth * column) : boxWidth,
          height: boxHeight,
          overflow: 'hidden'
        };
      ...
      getRamblingBox = function(boxWidth, boxHeight, row, column, settings, vars) {
        ...
        bottom = settings.alignBottom ? "" + (boxHeight * (settings.boxRows - (row + 1))) + "px" : 'auto';
