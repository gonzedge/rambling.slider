  ...
  (function($) {
    ...
    RamblingSlider = function(element, options) {
      ...
      ramblingRun = function(slider, kids, settings, nudge) {
        /*
              Get our vars
              */        var anims, box2Darr, boxes, colIndex, cols, current_effect, firstSlice, rowIndex, sliceStyle, slices, timeBuff, totalBoxes, v, _ref2, _results;
        ...
        if (['sliceDown', 'sliceDownRight', 'sliceDownLeft'].contains(current_effect)) {
          ...
          if (current_effect === 'sliceDownLeft') {
          ...
          v = 0;
          slices = slider.find('.rambling-slice');
          if (current_effect === 'sliceUpDownLeft') {
            slices = slices.reverse();
          }
          return slices.each(function() {
            var slice;
        ...
        } else if (current_effect === 'fold') {
          createSlices(slider, settings, vars);
          timeBuff = 0;
          i = 0;
          return slider.find('.rambling-slice').each(function() {
            var origWidth, slice;
            slice = $(this);
        ...
        } else if (current_effect === 'slideInRight') {
          createSlices(slider, settings, vars);
          firstSlice = slider.find('.rambling-slice:first');
          sliceStyle = {
            height: '100%',
        ...
        } else if (current_effect === 'slideInLeft') {
          createSlices(slider, settings, vars);
          firstSlice = slider.find('.rambling-slice:first');
          sliceStyle = {
            height: '100%',
        ...
        } else if (current_effect === 'boxRandom') {
          ...
          if (current_effect === 'boxRainReverse' || current_effect === 'boxRainGrowReverse') {
            boxes = boxes.reverse();
          }
          boxes.each(function() {
              ...
              _fn2 = function(rows) {
                if (prevCol >= 0 && prevCol < settings.boxCols) {
                  (function(row, col, time, i, totalBoxes) {
                    ...
                    w = box.width();
                    h = box.height();
                    if (current_effect === 'boxRainGrow' || current_effect === 'boxRainGrowReverse') {
