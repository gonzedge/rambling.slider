  ...
  (function($) {
    ...
    return RamblingSlider = function(element, options) {
      ...
      effect = function() {
        ...
        animationsToRun = ['sliceDownRight', 'sliceDownLeft', 'sliceDownRandom', 'sliceUpRight', 'sliceUpLeft', 'sliceUpRandom', 'sliceUpDown', 'sliceUpDownLeft', 'sliceUpDownRandom', 'foldRight', 'foldLeft', 'foldRandom', 'fade', 'slideInRight', 'slideInLeft', 'rolloverRight', 'rolloverLeft', 'boxRandom', 'boxRain', 'boxRainReverse', 'boxRainGrow', 'boxRainGrowReverse'];
      ...
      createSlices = function(slider, settings, vars, slideElement) {
        var i, _fn, _ref;
        ...
        _fn = function(i) {
          ...
          return animationContainer.append(getRamblingSlice(sliceWidth, i, settings.slices, vars, slideElement));
        };
      ...
      setSliderBackground = function() {
        ...
        if (!slider.find('.currentSlideElement').replaceWith(slideElement).length) {
          return slider.find('#rambling-animation').prepend(slideElement);
        }
      };
      ...
      animateFullImage = function(options) {
        var animate, hasFlash, image, slice, slices;
        hasFlash = vars.currentSlideElement.find('object,embed').length;
        slices = hasFlash ? createSlices(slider, settings, vars, vars.previousSlideElement) : createSlices(slider, settings, vars);
        slice = slices.first();
        if (settings.alignBottom) {
        ...
        animate = function(callback) {
          return slice.animate(options.animate || {
            ...
            if (callback) {
              return callback.apply(slice);
            }
          });
        };
        if (hasFlash) {
          setSliderBackground();
          return window.setTimeout(animate(function() {
            return this.remove();
          }), settings.speed * 1.5);
        } else {
          return animate();
      ...
      animations = {
        ...
        fade: function() {
          return animateFullImage(animationOptions.fadeIn);
        },
        fadeIn: function() {
          return animateFullImage(animationOptions.fadeIn);
        },
        fadeOut: function() {
          return animateFullImage(animationOptions.fadeOut);
        },
        slideIn: function() {
        ...
        rollover: function() {
          return animateFullImage(animationOptions.rolloverRight);
        },
        rolloverRight: function() {
          return animateFullImage(animationOptions.rolloverRight);
        },
        rolloverLeft: function() {
          return animateFullImage(animationOptions.rolloverLeft);
        },
      ...
      ramblingRun = function(slider, kids, settings, nudge) {
        ...
        vars.randomAnimation = settings.effect === 'random' || settings.effect.contains(',') ? getRandomAnimation() : null;
        vars.running = true;
        currentEffect = vars.currentSlideElement.find('object,embed').length ? 'fadeOut' : vars.randomAnimation || settings.effect;
