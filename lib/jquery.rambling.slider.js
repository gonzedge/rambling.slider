  ...
  */
  (function($) {
    ...
    return RamblingSlider = function(element, options) {
      var addCaption, addControlNavigation, addDirectionNavigation, addKeyboardNavigation, animateBoxes, animateFullImage, animateSlices, animationOptions, animationTimeBuffer, animations, animationsToRun, createBoxes, createSlices, foldSlices, getBox, getRamblingBox, getRamblingSlice, getRandomAnimation, getSlice, initialize, kids, pauseSlider, prepareAdaptiveSlider, prepareSliderChildren, processCaption, rainBoxes, ramblingRun, randomBoxes, resetTimer, run, setAnimationFinishedActions, setCurrentSlideImage, setSliderBackground, setSliderInitialState, settings, slideDownSlices, slideTo, slideUpDownSlices, slideUpSlices, slider, start, stop, timer, unpauseSlider, vars;
      slider = $(element);
      ...
      getRandomAnimation = function() {
        return animationsToRun[Math.floor(Math.random() * (animationsToRun.length + 1))] || 'fade';
      };
      ...
      ramblingRun = function(slider, kids, settings, nudge) {
        ...
        return animations[current_effect].apply(this);
      };
